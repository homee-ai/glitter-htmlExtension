                            ...
                            return gvc.bindView({
                                ...
                                onCreate:()=>{
                                    if(hover.indexOf(dd.id) !== -1){
                                        console.log('hover')
                                        setTimeout(()=>{
                                            const scrollTOP=(gvc.glitter.$('#'+gvc.id(component)).offset().top) - (gvc.glitter.$('body').offset().top) + (gvc.glitter.$('body').scrollTop())
                                            gvc.glitter.$('body').animate({ scrollTop: scrollTOP - gvc.glitter.$('body').height() / 2 }, 200);
                                        },100)
                                    }
...
${ (dd.expand || !option.return_) ? HtmlGenerate.editeInput({
                                            gvc:gvc,
                                            title:"模塊資源路徑",
                                            default:dd.js,
                                            placeHolder:"請輸入模塊路徑",
