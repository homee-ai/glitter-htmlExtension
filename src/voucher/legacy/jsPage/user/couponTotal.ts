...
init((gvc, glitter, gBundle) => {
    ...
    // [ Select, Preview, History ]
    const viewType = gBundle.viewType ?? 'Select';

    let vm: {
        empty: boolean;
        model: { backPoint: number; couponEnable: VoucherModel[] };
    } = {
        empty: true,
    ...
    Voucher.getVoucher(viewType, (data) => {
        ...
        if (data) {
            vm.model.couponEnable = data as VoucherModel[];
        } else {
                        ...
                        <main class="p-0" style="">
                            ...
                            ${gvc.bindView({
                                ...
                                view: () => {
                                    ...
                                    // if (vm.model.couponEnable.length == 0) {
                                    //     return `<div class="text-center mt-5">尚無優惠券</div>`;
                                    // }

                                    return gvc.map(
                                        vm.model.couponEnable.map((coupon, index) => {
                                            return gvc.bindView({
