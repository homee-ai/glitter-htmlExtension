...
export class ViewModel {
    ...
    public postView: (ideaPostData: any) => string;
    ...
    constructor(gvc: GVC) {
        ...
        function leaveModel(postData: IdeaData) {
         ...
         * */
        this.postView = (ideaPostData: IdeaData[]): string => {
            const gvc = this.gvc
            const glitter = gvc.glitter
            let userData = glitter.share.userData
...
<img src="img/more_vertical.png" style="width: 20px;height: 20px;" onclick="${gvc.event((e) => {
...
`
                    ...
                    ${gvc.bindView(() => {
                        return {
                            ...
                            view: () => {
                                console.log('scene:'+postData['scene'])
                                let isLike = (postData['dislike']) ? "img/sample/idea/like.svg" : "img/sample/idea/unlike.svg"
                                return ` <div class="me-auto d-flex">
                        <img class="likeIMG" src="${isLike}" style="width: 23px; height: 23px;margin-right: 16px;" onclick=" ${gvc.event(() => {
                                    ...
                                    ideaAPI.liked({
                                        user_id: glitter.share.userData.user_id,
                                        idea_id: key
                                    }, postData['dislike'], (result) => {
                                    })
                                    gvc.notifyDataChange(`toolBar${postData["idea_id"]}`)
                                    gvc.notifyDataChange(`likeCount${key}`)
                                })}">
                        <img src="img/sample/idea/boardChat.svg" style="width: 23px; height: 23px;margin-right: 16px;" onclick="${gvc.event(() => {
                                    changeToBoard(postData)
                                })}">
                        <img src="img/sample/idea/send.svg" style="width: 23px; height: 23px;;" onclick="">
                    </div>
                    ${(postData['scene'] === 'noImage') ? ``:` <img class="ms-auto" src="img/sample/idea/edit.svg" style="width: 23px; height: 23px;" onclick="${gvc.event(() => {
                                    leaveModel(postData)
                                })}">`}
                   `
                            },
