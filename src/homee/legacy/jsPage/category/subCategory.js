...
init((gvc, glitter, gBundle) => {
    ...
    let sortRow = [
        ...
        (() => {
            const map = {
                text: '價格', img: glitter.share.getLink('img/sample/category/sort.svg'), click: (e) => {
                    if (!origData) {
    ...
    glitter.share.selectSubCategory = (gBundle === null || gBundle === void 0 ? void 0 : gBundle.index) + 1 || 0;
    return {
        onCreateView: () => {
            let topInset = 0;
            let bottomInset = 0;
                        ...
                        <main style="background: white;padding-top: ${topInset - 20 + ((glitter.share.subCategoryVM.setSubCategoryRow === '') ? 125 : 150)}px;padding-left: 23px;padding-right: 23px;">
                            ...
                            view: () => {
                                if (glitter.share.subCategoryVM.productLoading) {
                                    let returnHTML = ``;
                                    glitter.share.productData.forEach((data, index) => {
