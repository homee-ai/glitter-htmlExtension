...
onclick="${gvc.event(() => {
                                            dd.config.json = JSON.stringify(dd.config)
                                                .replace('server_rout', 'serverRout')
                                                .replace('store_time', 'storeTime')
                                                .replace('space_image', 'spaceImage');
...
line-height: 17px;
text-align: center;
color: #858585;">儲存時間:${dd.date.substring(0, 10)}</span>
</div>
<img class="position-absolute top-0 right-0" src="${new URL('../img/more.svg', import.meta.url)}" style="width: 32px;height: 32px;right: 6px;">
</div>`;
                                    }).join('<div style="height: 16px;"></div>')}
${(() => {
                                        if (spaceData.length > 3) {
...
padding-top:0;padding-bottom:0;padding-left:0;padding-right:0;margin-top:0;margin-bottom:0;
margin-left:59px;margin-right:59px; bottom:25px;width:calc(100vw - 108px);height:48px;
